...
public class EamsRecordController extends BaseController {
     ...
     */
    @RequiresPermissions("evaluation:record:detail")
    @GetMapping("/detail")
    public String detail(String stuNum, Long ruleId, Model model) {
        ...
        model.addAttribute("professionName", professionService.selectProfessionFullNameByProfId(student.getProfId()));
    ...
    @Log(title = "评优评奖记录", businessType = BusinessType.EXPORT)
    @RequiresPermissions("evaluation:record:export")
    @PostMapping("/export")
    @ResponseBody
    public AjaxResult export(RecordHeader recordHeader) {
        ...
        return util.exportExcel(list, "评优评奖记录");
    }
